...
void CSE_7759B::readValuesFromDevice() {
  ...
  double _voltage = sensor->getVoltage();
  double _active = sensor->getActivePower();
  double _apparent = _voltage * _current;
  ...
  setVoltage(0, static_cast<int>(_voltage * 100.0));            // voltage in 0.01 V
  setCurrent(0, static_cast<int>(_current * 1000.0));           // current in 0.001 A
  setPowerActive(0, static_cast<int>(_active * 100000.0));      // power in 0.00001 kW
  setFwdActEnergy(0, energy);               // energy in 0.00001 kWh
  setPowerApparent(0, static_cast<int>(_apparent * 100000.0));  // power in 0.00001 kVA
  setPowerReactive(0, static_cast<int>(_reactive * 100000.0));  // power in 0.00001 kvar
  setPowerFactor(0, static_cast<int>(_pf * 1000.0));            // power in 0.001
