...
void setup() {
  ...
  for (uint8_t nr = 0; nr < ConfigManager->get(KEY_MAX_ANALOG_READING)->getValueInt(); nr++) {
    ...
    if (gpio != OFF_GPIO) {
      Supla::GUI::analogSensorData.push_back(new Supla::Sensor::AnalogReading(gpio));
#ifdef SUPLA_CONDITIONS
  ...
  if (pinRX1 != OFF_GPIO && pinTX1 != OFF_GPIO) {
    ...
    PZEMv3 = new Supla::Sensor::PZEMv3(&Serial, pinRX1, pinTX1, 0xF8);
#else
    PZEMv3 = new Supla::Sensor::PZEMv3(pinRX1, pinTX1, 0xF8);
#endif
  }
